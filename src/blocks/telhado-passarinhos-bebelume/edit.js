...
import { PanelBody, ToggleControl, ColorPicker, TextControl } from '@wordpress/components';
import { useEffect } from '@wordpress/element';
import { useSelect } from '@wordpress/data';

export default function Edit({ attributes, setAttributes, clientId }) {
  const { backgroundColor, link, invertLayout, hasImage, showClouds, showGarden } = attributes;
  const blockProps = useBlockProps({ className: 'editor-telhado-passarinhos-bebelume' });
  ...
  return (
    <>
      <InspectorControls>
        <PanelBody title={__('Configurações do Telhado Passarinhos Bebelume', 'bebelume-theme')}>
      ...
      <div {...blockProps}>
        <div
          className="preview-telhado-passarinhos-bebelume"
          style={{
        ...
        >
          ...
          <div style={{ transform: invertLayout ? 'scaleX(-1)' : 'none' }}>
            ...
            >
              🏠 {__('Telhado Passarinhos Bebelume', 'bebelume-theme')}
            </div>
