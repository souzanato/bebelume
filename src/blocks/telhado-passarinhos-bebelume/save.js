...
export default function Save({ attributes }) {
  ...
  const andarClasses = `bebelume-andar telhado-passarinhos-bebelume ${invertLayout ? 'invertido' : ''} ${hasImage ? 'has-image' : ''}`.trim();
  return (
    <section {...blockProps}>
      {showClouds && (
      ...
      {link ? (
        <a href={link} className={`${andarClasses} d-block text-decoration-none`}>
          {content}
        </a>
      ) : (
        <div className={andarClasses}>
          {content}
        </div>
      )}
    </section>
